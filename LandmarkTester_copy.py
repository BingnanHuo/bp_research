...
from matrix_utils import normalize_ubyte, preprocess_img, arr_info
from sklearn.metrics import mean_squared_error

class LandmarkTester:
    def __init__(self, input_image, model_to_use="MEE"):
        super().__init__()
        self._image = input_image
        self._model = model_to_use

        start_time = time.time()
        self._og_landmarks = GetLandmarks(self._image, model_to_use)._shape
        end_time = time.time()
        ...
        self._test_params2 = None

        #self._rotated_images = None # No longer used
        self._test_landmarks = None # shape: (num_tests, num_landmarks, 2)
        self._median_landmarks = None
        
        self._points_dist = None # shape: (num_landmarks,num_tests, 2)
        self._xy_dist = None # shape: (num_landmarks, 2, num_tests)
...
################################  Random Tests  #################################
    ...
    def test_combined(self, rand_angle=7, scale=25, num_test=30, rand_mode='normal', per_channel=True):
        ...
        if dist == 'uniform':
            if per_channel is True:
                self._test_params2 = (2*(np.random.rand(*((num_test,)+self._image.shape))-0.5)*scale).astype(np.half)
            else:
                one_chan = 2*(np.random.rand(*((num_test,)+self._image.shape[:2]))-0.5)*scale
                self._test_params2 = np.stack([one_chan, one_chan, one_chan], axis=-1).astype(np.half)
                
        elif dist == 'normal' or dist == 'laplace' or dist == 'poisson':
            if per_channel is True:
                self._test_params2 = np.random.normal(loc=0, scale=scale, size=(num_test,)+self._image.shape).astype(np.half)
            else:
                one_chan = (np.random.normal(loc=0, scale=scale, size=(num_test,)+self._image.shape[:2]))
                self._test_params2 = np.stack([one_chan, one_chan, one_chan], axis=-1).astype(np.half)    
        else:
...
###################################  Tools  #####################################
    ...
    def noise(self, added_noise):
        ...
        new_img = (np.clip(np.uint16(self._image) + added_noise, 0, 255)).astype(np.uint8)
        print("One test has completed...")
