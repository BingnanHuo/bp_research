...
Modified by: Bingnan (Nick) Huo -- nick.huo at bucknell.edu
Modified Date: Summer 2022
"""
...
class GetLandmarks:
    ...
    def getlandmarks(self):
        ...
        if d > 1:
            if self._colored == False:
                #transform to gray 
                gray=cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
            else:
                gray = image[...,::-1].copy() #flipping the color channels: BGR -> RGB
            
        #resize to speed up face detection
        newWidth=self._resolution
        ...
        if len(rects) == 1:   
            ...
            for (i, rect) in enumerate(rects):
                ...
                self._boundingbox=[int(rect.left() * ScalingFactor), 
                                   int(rect.top() * ScalingFactor),
                                   int(rect.right() * ScalingFactor),
                                   int(rect.bottom() * ScalingFactor)]
            ...
            #emit an empty array 
            print('ERROR: Cannot find face or multiple faces detected!')
            return
